...
import cc.yongdream.nshx.AndroidVideoApi5JniWrapper;
...
public class mainActivity extends Cocos2dxActivity{
	...
	private native void setVideoWindows(Object wid);

	private native void setVideoPreviewWindowId(Object pid);

	private native void startVideo(String apurl, String arurl, String vpurl,
			String vrurl);

	private native void stopVideo();

	private native void switchVCapture();

	// 主播播放
	private native void startAVPlay(String apurl, String vpurl);

	private native void stopAVPlay();

	// 单条流双向
	private native void startPhone(String rurl, String purl);

	private native void stopPhone();

	// 上行
	private native void openAVSingleRecord(String arurl, String jpgfile);

	private native void startAVSingleRecord();

	private native double stopAVSingleRecord();

	// 开始录制文件
	// private native void recordAVSingleStart();

	private native void startRoomPlay(String arurl);

	private native void stopRoomPlay();

	private native void setLog(int i);

	private native void convertVideoFormat(String source, String dest);
...
// 	    url = "rtmp://10.0.37.49/live/test00002";
 	    if(url.indexOf("rtmp://")!=-1){
 	    	openWebView();
 	    	girlfan.startRoomPlay(url);
 	    	girlfan.setVideoWindows(mVideoWindow);
 	    }
 	    else{
...
//    	this.setCutType("2");
 	    this.runOnUiThread(new Runnable() {//在主线程里添控件                                                                                        
           public void run() {     
				if (mVideoRenderingView == null)
					return ;
			   mVideoRenderingView.setZOrderOnTop(false);
...
//	    	this.runOnUiThread(new Runnable() {//在主线程里添控件                                                                                        
//	            public void run() {  
//	               Log.e("GirlFan", "mVideoRenderingView set bottom");
//	 			   mVideoRenderingView.setZOrderOnTop(false);
//	         	   mVideoRenderingView=null;  
//	            }
//	  	   	});
...
//								setVideoWindows(null);
							...
							public void onVideoPreviewSurfaceReady(
									AndroidVideoWindowImpl vw, SurfaceView sv) {
								setVideoPreviewWindowId(sv);
							};
