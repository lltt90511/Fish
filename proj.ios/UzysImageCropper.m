...
- (id)initWithImage:(UIImage*)newImage andframeSize:(CGSize)frameSize andcropSize:(CGSize)cropSize
{
    ...
    if(self)
    {
        ...
        _imageScale = 320/cropSize.width ;
        
        CGRect imgViewBound = CGRectMake(0, 0, _inputImage.size.width, _inputImage.size.height);   //이미지가 생성될 사이즈.
        ...
        _cropperView.backgroundColor = [UIColor grayColor];
        _cropperView.alpha = 0;
        ...
        {
            int w = (i/bytes_per_pix)%width;
            int h = (i/bytes_per_pix)/width;
            if (w >= (width-_imageScale*cropSize.width)/2 && w <= width-(width-_imageScale*cropSize.width)/2
                && h >= (height-_imageScale*cropSize.height)/2 && h <= height-(height-_imageScale*cropSize.height)/2) {
                rgbbuffer[i+3] = 255;//这个是alfa值，因为是小字节序所以ARGB的内存储存方式为BGRA
                rgbbuffer[i+0] = 0;
                rgbbuffer[i+1] = 0;
                rgbbuffer[i+2] = 0;
            }else {
        ...
        _maskingLayer.frame = bgView.bounds;
        [_maskingLayer setContents:(id)frame];
        [bgView.layer setMask:_maskingLayer];
...
- (UIImage*) getCroppedImage
{
    ...
    CGSize CropinViewSize = CGSizeMake((CropinView.size.width*(1)),(CropinView.size.height*(1)));
    ...
    CGRect CropRectinImage = CGRectMake((NSInteger)(CropinView.origin.x * (1)) ,(NSInteger)( CropinView.origin.y * (1)), (NSInteger)CropinViewSize.width,(NSInteger)CropinViewSize.height);
