...
printTable = function(root)
	...
	local function _dump(t,space,name)
		...
		for k,v in pairs(t) do
			...
			if cache[v] then
				local str = "+" .. key .. " {" .. cache[v].."}"
				table.insert(temp,str)
				strLen = strLen + string.len(str)
			elseif type(v) == "table" then
				local new_key = name .. "." .. key
				cache[v] = new_key
				local str = "+" .. key .. _dump(v,space .. (next(t,k) and "|" or " " ).. string.rep(" ",#key),new_key)
				table.insert(temp,str)
				strLen = strLen + string.len(str)
			else
				local str = "+" .. key .. " [" .. tostring(v).."]"
				table.insert(temp,str)
				strLen = strLen + string.len(str)
			end
			if strLen > 20000 then
				flag = true
				--print ("table dump String is too long,not print here!")
				return ""
			end
			if flag then
				--print ("table dump String is too long,not print here!")
	...
	if flag then
		print ("@@@@@@@@@@@@@@@@table dump String is too long,not print here!", debug.traceback())
		return
	end
